...
export const modelDefinitions = [
  ...
  { id: 'image', name: 'Qwen Image 2.1', subtitle: '0xSojalSec · Q8', purpose: 'Create images', description: 'Turn ideas into pictures and transform your reference images.', ids: ['diffusion','encoder','vae','image-projector'], catalog: 'model-manifest.json', stateFile: 'models/installed.json' },
  { id: 'video', name: 'Sulphur 2', subtitle: 'Local video', purpose: 'Make videos', description: 'Create short clips with motion, sound, and image references.', ids: ['video','video-encoder','video-lora'], catalog: 'video-model-manifest.json', stateFile: 'models/video-installed.json' },
...
export class ModelManager extends VideoRuntime {
  ...
  async refresh() {
    ...
    this.items = await Promise.all(modelDefinitions.map(async def => {
      ...
      const engine = def.id === 'chat' ? platform.ollamaExe : def.id === 'image' ? Object.values(platform.engines).find(Boolean) : def.id === 'video' ? video.python : def.id === 'music' ? audio.music?.python : audio[def.id];
      ...
      const supported = (def.id !== 'video' || videoSupported()) && !(def.audio && process.platform === 'darwin' && process.arch === 'x64');
      return { id: def.id, name: def.name, subtitle: def.subtitle, purpose: def.purpose, description: def.description, bytes: files.reduce((n,f) => n + f.bytes,0), installed, weights, active: installed && this.preferences.active[def.id] !== false, supported, requirement: supported ? null : def.audio ? 'The current audio runtime requires Windows, Linux, or Apple Silicon' : 'NVIDIA CUDA on Windows or Linux required', update: this.checks[def.id]?.state || 'unchecked', checkedAt: this.checks[def.id]?.at, checkError: this.checks[def.id]?.error };
    }));
    return this.snapshot();
  }
  snapshot() { return { models: this.items, job: this.job, operating: !!this.operating, checking: this.checking, setupRequired: !this.items.some(m => m.installed), active: this.items.filter(m => m.active).map(m => m.id) }; }
  has(id) { return this.items.some(m => m.id === id && m.active); }
  async checkUpdates() {
  ...
  async install(ids) {
    this.assertIdle();
    if (!Array.isArray(ids) || !ids.length || ids.length > 6 || new Set(ids).size !== ids.length || ids.some(id => !this.items.some(m => m.id === id && m.supported))) throw Object.assign(new Error('Select at least one supported model.'), { status:400 });
  ...
  async uninstall(id, confirmation) {
    ...
    try {
      ...
      const ownedFolder = def.audio ? join(root,'audio',id) : id === 'image' || id === 'video' ? join(root,id) : null;
      if (ownedFolder) {
        try {
